...
const Pagination: FC<PaginationProps> = ({ currentPage, totalPages, perPage: currentPerPage, handleParamsChange }) => {
   ...
   useEffect(() => {
      handleParamsChange({ page: '1', limit: debouncedPerPage.toString() });
   }, [handleParamsChange, debouncedPerPage]);
   useEffect(() => {
      handleParamsChange({ page: debouncedPage.toString() });
   }, [handleParamsChange, debouncedPage]);

   const paginationStart = useMemo(() => Math.ceil(page / PAGES_PER_BLOCK), [page]);

   return (
      <div className="w-full flex xl:justify-end max-sm:flex-col max-sm:gap-y-2 gap-x-1">
         <select
            value={perPage}
            onChange={handlePerPageChange}
            className="w-fit border border-gray-300 text-gray-900 text-sm rounded-lg outline-none focus:ring-1 focus:ring-black block py-2.5 pl-2.5"
         >
